...
const { data: activeProjectNews } = useAsyncData(
  () => `upcoming-project-news:${activeSlug.value}`,
  async () => {
...
const newsItemsForActive = computed(() =>
  (activeProjectNews.value || []).map((post) => ({
    ...
    link: `/news/${post.slug}?source=project&project_status=upcoming`,
  }))
)
const fallbackPageNews = computed(() => {
  const pageNews = activeProjectPage.value?.news
  if (!Array.isArray(pageNews)) return []
  return pageNews.map((post) => ({
    id: post.id,
    title: post.title,
    summary: stripHtml(post.content || '').slice(0, 180) + (stripHtml(post.content || '').length > 180 ? '...' : ''),
    date: post.created_at,
...
const updatesForActive = computed(() => {
  if (newsItemsForActive.value.length) return newsItemsForActive.value
...
const totalPages = computed(() => Math.max(1, Math.ceil(updatesForActive.value.length / pageSize.value)))
const paginatedNews = computed(() => {
  const start = (currentPage.value - 1) * pageSize.value
  return updatesForActive.value.slice(start, start + pageSize.value)
})
...
watch(projectGroupsWithSlugs, (groups) => {
  ...
  if (!activeTab.value) {
    ...
    if (fromHash) {
      updateActiveTabFromHash(fromHash)
    }
    if (!activeTab.value) {
      activeTab.value = bySlug || firstProjectItem?.id || ''
    }
...
function updateActiveTabFromHash(hash) {
  const pickPreferredSubgroupItem = (items = []) => {
    const onlyProjects = items.filter((item) => item?.slug)
    return onlyProjects.find((item) => item?.is_news_landing) || onlyProjects[0] || null
  }

  for (const group of projectGroupsWithSlugs.value) {
    for (const subgroup of group.subgroups || []) {
      if (subgroup.id === hash) {
        const firstItem = pickPreferredSubgroupItem(subgroup.items || [])
      ...
      <div v-if="isNewsLanding" class="space-y-10">
        ...
        <section class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div class="mt-2 text-2xl font-bold text-emerald-700">{{ activeProjectPage?.project?.status?.name || 'Upcoming' }}</div>
          </div>
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="text-sm text-gray-500">Funding</div>
            <div class="mt-2 text-2xl font-bold text-emerald-700">{{ activeProjectPage?.project?.funding_type?.name || 'N/A' }}</div>
          </div>
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="text-sm text-gray-500">Program</div>
            <div class="mt-2 text-2xl font-bold text-emerald-700">{{ activeProjectPage?.project?.program_group || 'N/A' }}</div>
          </div>
        </section>
      </div>
