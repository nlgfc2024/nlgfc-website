...
async function downloadRepository(url, cwd, { headers } = {}) {
  const tarFile = pathe.join(cwd, ".content.clone.tar.gz");
  ...
  try {
    ...
    const stream = node_fs.createWriteStream(tarFile);
    ...
    await promises.rm(tarFile, { force: true });
  }
}
function parseGitHubUrl(url) {
  ...
  if (match) {
    ...
    if (["fix", "feat", "chore", "test", "docs"].includes(branch)) {
      ...
      branch = pathe.join(branch, pathParts[0]);
    ...
    getContents: ({ app, nuxt, options }) => {
      const componentsMap = app.components.filter((c) => {
        ...
        map[c.pascalName] = map[c.pascalName] || [
          c.pascalName,
          `${knitwork.genDynamicImport(pathe.isAbsolute(c.filePath) ? "./" + pathe.relative(pathe.join(nuxt.options.buildDir, "content"), c.filePath).replace(/\b\.(?!vue)\w+$/g, "") : c.filePath.replace(/\b\.(?!vue)\w+$/g, ""), { wrapper: false, singleQuotes: true })}`,
          c.global
        ];
        return map;
      }, {});
...
async function startSocketServer(nuxt, options, manifest) {
  ...
  if (websocketOptions.enabled) {
    nuxt.hook("nitro:init", async (nitro) => {
      ...
      listener = await listhen.listen(() => "Nuxt Content", websocketOptions);
...
function defineLocalSource(source) {
  ...
  const resolvedSource = {
    ...
    getKeys: async () => {
      const _keys = await tinyglobby.glob(source.include, { cwd: resolvedSource.cwd, ignore: source.exclude || [], dot: true, expandDirectories: false }).catch(() => []);
...
z$1.z.ZodType.prototype.editor = function(options) {
  this._def.editor = { ...this._def.editor, ...options };
  return this;
};
const z = z$1.z;
function zodToStandardSchema(schema, name) {
  const jsonSchema = zodToJsonSchema.zodToJsonSchema(schema, name);
...
async function installMDCModule(contentOptions, nuxt) {
  ...
  await kit.installModule("@nuxtjs/mdc");
...
const nuxthub = definePreset({
  ...
  async setupNitro(nitroConfig, options) {
    ...
    if (nitroConfig.dev === false) {
      await promises.mkdir(pathe.resolve(nitroConfig.rootDir, ".data/hub/database/queries"), { recursive: true });
      let i = 1;
      ...
      for (const dumpFile of dumpFiles) {
        await promises.writeFile(pathe.resolve(nitroConfig.rootDir, ".data/hub/database/queries", dumpFile.file), dumpFile.content);
      }
...
const module$1 = kit.defineNuxtModule({
  ...
  async setup(options, nuxt) {
    ...
    nuxt.options.alias["#content/components"] = kit.addTemplate(componentsManifestTemplate(manifest)).dst;
    nuxt.options.alias["#content/manifest"] = kit.addTemplate(manifestTemplate(manifest)).dst;
    const typesTemplateDst = kit.addTypeTemplate(contentTypesTemplate(manifest.collections)).dst;
    nuxt.options.nitro.typescript ||= {};
    nuxt.options.nitro.typescript.tsConfig = defu__default(nuxt.options.nitro.typescript.tsConfig, {
...
async function processCollectionItems(nuxt, collections, options) {
  ...
  ].map((tag) => getMappedTag(tag, options?.renderer?.alias)).filter((tag) => !htmlTags__default.includes(scule.kebabCase(tag))).map((tag) => scule.pascalCase(tag));
