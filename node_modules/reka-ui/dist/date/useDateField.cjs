...
function daySegmentAttrs(props) {
  ...
  const valueMin = 1;
  const valueMax = date_comparators.getDaysInMonth(date);
...
function monthSegmentAttrs(props) {
  ...
  const valueMin = 1;
  const valueMax = 12;
  const valueText = isEmpty ? "Empty" : `${valueNow} - ${formatter.fullMonth(date_comparators.toDate(date))}`;
  return {
    ...commonSegmentAttrs(props),
    "aria-label": "month, ",
    "contenteditable": true,
...
function useDateField(props) {
  ...
  const attributes = vue.computed(() => segmentBuilders[props.part]?.attrs({
  ...
  function handleDaySegmentKeydown(e) {
    if (!date_segment.isAcceptableSegmentKey(e.key) || date_segment.isSegmentNavigationKey(e.key))
      return;
    ...
    if (date_segment.isNumberString(e.key)) {
      const num = Number.parseInt(e.key);
      const segmentMonthValue = props.segmentValues.value.month;
      const daysInMonth = segmentMonthValue ? date_comparators.getDaysInMonth(props.placeholder.value.set({ month: segmentMonthValue })) : 31;
  ...
  function handleHourSegmentKeydown(e) {
    ...
    if (!date_segment.isAcceptableSegmentKey(e.key) || date_segment.isSegmentNavigationKey(e.key) || !("hour" in dateRef) || !("hour" in props.segmentValues.value))
      return;
  ...
  function handleMinuteSegmentKeydown(e) {
    const dateRef = props.placeholder.value;
    if (!date_segment.isAcceptableSegmentKey(e.key) || date_segment.isSegmentNavigationKey(e.key) || !("minute" in dateRef) || !("minute" in props.segmentValues.value))
      return;
  ...
  function handleSecondSegmentKeydown(e) {
    const dateRef = props.placeholder.value;
    if (!date_segment.isAcceptableSegmentKey(e.key) || date_segment.isSegmentNavigationKey(e.key) || !("second" in dateRef) || !("second" in props.segmentValues.value))
      return;
  ...
  function handleDayPeriodSegmentKeydown(e) {
    if ((!date_segment.isAcceptableSegmentKey(e.key) || date_segment.isSegmentNavigationKey(e.key)) && e.key !== "a" && e.key !== "p" || !("hour" in props.placeholder.value) || !("dayPeriod" in props.segmentValues.value))
      return;
    if (e.key === kbd.ARROW_UP || e.key === kbd.ARROW_DOWN) {
  ...
  function handleSegmentKeydown(e) {
    ...
    if (![kbd.ARROW_LEFT, kbd.ARROW_RIGHT].includes(e.key) && e.key !== kbd.TAB && e.key !== kbd.SHIFT && date_segment.isAcceptableSegmentKey(e.key)) {
