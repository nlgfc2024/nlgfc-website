...
const detectActiveItemFromHash = () => {
  ...
  if (props.sidebarType === 'projects' && route.hash) {
    ...
    for (const group of props.sectionsData) {
      ...
      if (matchingItem) {
        // Only emit if it's different from current activeId
...
// This ensures that if activeId is set by the parent, the correct group is expanded.
...
const setActiveItem = (id) => {
  ...
  // isSidebarOpen.value = false; // Removed for automatic scroll-based toggling
...
const toggleGroup = (groupName) => {
  ...
  if (openGroup.value !== groupName) {
    ...
    if (groupToOpen && groupToOpen.items && groupToOpen.items.length > 0) {
      setActiveItem(groupToOpen.items[0].id); // This will also update the route hash
    }
  } else {
...
// Computed property to find which group contains the active item
const activeGroup = computed(() => {
  if (props.sidebarType === 'projects' && props.activeId) {
    for (const group of props.sectionsData) {
      if (group.items.some(item => item.id === props.activeId)) {
        return group.group;
      }
    }
  }
...
<template>
  <div>
    <div
      ...
      <nav class="flex-grow overflow-y-auto space-y-2 pr-2"> <template v-if="sidebarType === 'opportunities'">
          <NuxtLink
